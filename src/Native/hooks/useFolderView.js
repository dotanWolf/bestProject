...
export function useFolderView(currentFolderId = null) {
  ...
  const { allFiles, refreshFiles, token, currentUserId, loading: contextLoading } = useFiles();
  ...
  const refreshCurrentFolder = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`http://${IP}:8080/api/files/folders/${targetId}`, {
        headers: {
  ...
  const refreshCorrectList = async () => {
    if (targetId) await refreshCurrentFolder(); // Refresh local folder
  ...
  const handleRename = (file) => {
    if (Platform.OS === 'ios') {
      Alert.prompt("Rename", `Rename ${file.name}`, [
          { text: "Cancel", style: "cancel" },
          { text: "Rename", onPress: async (n) => {
               if(!n) return;
               await fetch(`http://${IP}:8080/api/files/${file.id}`, {
                 method: "PATCH",
                 headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}`, userid: currentUserId },
  ...
  const handleStar = async (file) => {
        const isOwner = file.ownerId === currentUserId;
        const url = isOwner ? `http://${IP}:8080/api/files/${file.id}` : `http://${IP}:8080/api/files/${file.id}/permissions/${file.permissionId}`;
        await fetch(url, { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}`, userid: currentUserId }, body: JSON.stringify({ isStarred: !file.isStarred }) });
        refreshCorrectList();
  };

  const handleOpenPermissions = (file) => { setSelectedFile(file); setShowPermissions(true); };

  return { entries, loading, isAddOpen, setIsAddOpen, showPermissions, setShowPermissions, selectedFile, handleDelete, handleRename, handleStar, handleFileUpload, handleCreateFolder, handleOpenPermissions };
}